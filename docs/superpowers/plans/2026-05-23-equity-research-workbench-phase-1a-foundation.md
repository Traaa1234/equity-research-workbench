...
## Milestone 2: Database Schema + Neon + Stack Auth

Goal: Neon project provisioned, all tables created via Drizzle migration, RLS policies applied using the `current_user_id()` pattern, integration tests verify RLS isolates users.

**Prerequisite (manual, one-time):**

Before any task in this milestone, the developer must:

1. Create a Neon account at https://neon.tech, create a project (any region). Copy the connection string from the "Connection Details" pane — this is `DATABASE_URL`.
2. In Neon SQL editor, create a service role and capture a second connection string for it. Run:
   ```sql
   create role service_role with login password '<long-random-pw>' bypassrls;
   grant connect on database neondb to service_role;
   grant all on schema public to service_role;
   alter default privileges in schema public grant all on tables to service_role;
   alter default privileges in schema public grant all on sequences to service_role;
   ```
   Then construct the connection string for `service_role` by replacing the username and password in the original `DATABASE_URL` — that's `DATABASE_URL_SERVICE_ROLE`.
3. Create a Stack Auth account at https://stack-auth.com, create a project. Copy the three keys (project id, publishable client key, secret server key) into `.env.local`.
4. Copy `.env.example` to `.env.local` and fill in the values from steps 1-3, plus Financial Datasets + Upstash + `CRON_SECRET`.

These are documented in `.env.example`; no scripting required.

### Task 2.1: Verify Neon connectivity and create the `authenticated` role

**Files:** None (Neon SQL editor only).

- [ ] **Step 1: Confirm `.env.local` is populated**

From the project root, in PowerShell:
```powershell
cd C:\Users\elinw\Projects\equity-research-workbench
Get-Content .env.local | Select-String "DATABASE_URL|STACK"
```
Expected: all four lines non-empty (`DATABASE_URL=`, `DATABASE_URL_SERVICE_ROLE=`, two `*STACK*` keys).

- [ ] **Step 2: Create the `authenticated` role in Neon**

In Neon's SQL editor (web UI), run:
```sql
create role authenticated nologin;
grant authenticated to <owner_role_name>;  -- replace with the role name Neon uses for your DATABASE_URL user
grant connect on database neondb to authenticated;
grant usage on schema public to authenticated;
alter default privileges in schema public grant select on tables to authenticated;
```

The `authenticated` role is the role our `withUserContext` helper will impersonate (via `SET ROLE`) for user-scoped queries. It has SELECT on tables — RLS policies further restrict what individual users can see.

- [ ] **Step 3: Verify both connection strings work**

Install `psql` only if you want to do this from the command line; otherwise use Neon's web SQL editor and just confirm you can run `select 1;`. Don't waste time installing psql.

- [ ] **Step 4: Commit** (only the documentation updates, since no code changed yet)

This task adds no files; skip the commit. Documentation is already in `.env.example`.

---

### Task 2.2: Install Drizzle + Postgres client + Stack Auth + jose
...
```bash
cd "C:/Users/elinw/Projects/equity-research-workbench"
pnpm add drizzle-orm postgres @stackframe/stack jose
```

- `drizzle-orm` — ORM + query builder.
- `postgres` — Postgres driver. We use the standard TCP driver, not Neon's serverless HTTP driver, for Node-runtime route handlers in Slice 1.
- `@stackframe/stack` — Stack Auth SDK with Next.js App Router integration.
- `jose` — JWT signing for integration tests (we sign a test JWT and `SET LOCAL request.jwt.claim.sub` to verify RLS in isolation from Stack Auth's network).
...
```bash
pnpm list --depth 0 | grep -E "drizzle|postgres|stackframe|jose"
```
...
git commit -m "chore: add Drizzle, postgres-js, Stack Auth, and jose"
```

---

### Task 2.3: Write Drizzle schema for `companies` and Drizzle config

**Files:**
- Create: `drizzle.config.ts`
- Create: `lib/db/schema.ts` (companies only)
...
We use the service-role connection for migrations because they create policies and need elevated permissions.

`dotenv` is not yet installed; add it now:
...
- [ ] **Step 2: Write `lib/db/schema.ts` with `companies` only**
...
pnpm db:generate
```
Expected: creates `lib/db/migrations/0000_<random>.sql` with `CREATE TABLE companies`.

- [ ] **Step 4: Apply the migration to Neon via `drizzle-kit push`**
...
When prompted, confirm the table creation. Expected: "Changes applied".

- [ ] **Step 5: Verify the table exists**

In Neon SQL editor: `select * from information_schema.tables where table_schema = 'public';` — should list `companies`.

- [ ] **Step 6: Commit**

```bash
cd "C:/Users/elinw/Projects/equity-research-workbench"
git add drizzle.config.ts lib/db/schema.ts lib/db/migrations/ package.json pnpm-lock.yaml
git commit -m "feat(db): add companies table with Drizzle schema"
```

---

### Task 2.4: Add `snapshots` table
...
Consolidate the imports at the top of the file (`numeric` joins the existing `drizzle-orm/pg-core` import).
...
### Task 2.7: RLS policies via `current_user_id()` and service-role grants

**Files:** Create a hand-written SQL migration `lib/db/migrations/9999_rls_policies.sql` and apply it manually via Drizzle's migration loader (or directly via Neon's SQL editor for Slice 1; we'll formalize this in Phase 1C with `drizzle-kit migrate`).

- [ ] **Step 1: Write `lib/db/migrations/9999_rls_policies.sql`**
...
- [ ] **Step 2: Apply manually via Neon SQL editor**

Paste the contents of the file into Neon's SQL editor and run. Confirm no errors.
...
```
Expected: `rowsecurity = true` for all 8 app tables.

- [ ] **Step 4: Commit**

```bash
cd "C:/Users/elinw/Projects/equity-research-workbench"
git add lib/db/migrations/9999_rls_policies.sql
git commit -m "feat(db): RLS policies via current_user_id() helper"
```

---

### Task 2.8: Write `lib/db/client.ts` — Drizzle clients + `withUserContext` helper
...
/**
 * User-scoped Drizzle client. Subject to RLS. Connection uses the
 * `authenticated` Postgres role and sets `request.jwt.claim.sub` to the
 * Stack Auth user id at the start of each transaction.
 *
 * Call sites should always use `withUserContext(userId, ...)` rather than
 * holding the client directly, so the claim is guaranteed to be set.
 */
...
/**
 * Run a function against the user-scoped DB with `request.jwt.claim.sub`
 * set to `userId` for the duration of the transaction. RLS policies see
 * this via `current_user_id()`.
 */
export async function withUserContext<T>(
  ...
  return db.transaction(async (tx) => {
    ...
    // `tx` is typed the same as `db` for purposes of querying.
...
git commit -m "feat(db): service + authenticated clients with withUserContext"
```
...
**Files:** Create `tests/helpers/test-db.ts`.

For RLS tests we don't need Stack Auth — RLS policies only consult `current_user_id()` which reads the session-local claim. So a test "user" is just a fresh uuid, and the helper exposes `withUserContext` directly.
...
/**
 * Build a fresh user-scoped client for a given user id. Equivalent to what
 * `withUserContext` does in app code, but exposes the transaction so callers
 * can run multiple queries inside one `request.jwt.claim.sub` setting.
 */
export function makeTestUserDb() {
  ...
  return {
    /** Open a transaction with `request.jwt.claim.sub` set to `userId` and run `fn`. */
...
/** Truncate all app tables (uses service-role client). */
...
git commit -m "test: Neon test helpers with user-scoped and service-role clients"
```
...
**Files:** Create `tests/integration/rls-watchlist.test.ts`.
...
- [ ] **Step 2: Run, verify it passes against the live Neon DB**

```bash
cd "C:/Users/elinw/Projects/equity-research-workbench"
pnpm test:integration tests/integration/rls-watchlist.test.ts
```

Expected: 3 passed. (If RLS test fails with "row violates row-level security policy" on the insert in `beforeEach`, then the `authenticated` role is missing INSERT/SELECT grants — go back to Task 2.7 and re-apply.)
